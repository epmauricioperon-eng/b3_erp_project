...
class ProventosController:
    ...
    def __init__(self):
        # Instanciamos nossas ferramentas de trabalho
    ...
    def calcular_proventos_a_receber(self) -> pd.DataFrame:
        ...
        for ticker in ativos_na_carteira:
            ...
            try:
                ultimo_anuncio = df_dividendos.iloc[0]
                
                data_com_str = str(ultimo_anuncio.get('DATA COM', ''))
                data_pagamento_str = str(ultimo_anuncio.get('Pagamento', ''))
                valor_str = ultimo_anuncio.get('Valor', 0.0)
                
                if not data_com_str or str(data_com_str).upper() == 'NAN':
                    continue

                # Filtro Temporal: Só mostra na previsão se a data ainda não chegou
                data_pagamento_dt = pd.to_datetime(data_pagamento_str, format="%d/%m/%Y").date()
                hoje = datetime.now().date()

                if data_pagamento_dt <= hoje:
                    continue # Já passou ou é hoje, então vai para a Caixa de Conciliação, não para previsão

                valor_rendimento = float(str(valor_str).replace(',', '.'))
    ...
    def obter_proventos_pendentes_de_confirmacao(self) -> list:
        ...
        for ticker in ativos_na_carteira:
            ...
            try:
                ...
                data_pagamento_str = str(ultimo_anuncio.get('Pagamento', ''))
                valor_str = ultimo_anuncio.get('Valor', 0.0)
                tipo_str = str(ultimo_anuncio.get('Tipo', 'Rendimento')).upper()
                ...
                # Regra: O pagamento é hoje ou já passou
                if data_pagamento_dt <= hoje:
                    
                    # Formata a data para YYYY-MM-DD (padrão do banco)
